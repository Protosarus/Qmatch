...
| Discover ranking | Trusted structural L2 (`structural_l2_v1`, canonical 20D). `CompatibilityScoring` is **rollback only** (`legacy_v1`). No live %. Persona/archetype are not Matching keys. Discover L3 v1 and L4 v1 = **non-ranking diagnostics**; L5 v1 = **validated research shadow** (`validated_shadow_not_live`, mixed-state QI only) | RUNTIME_WIRED |
...
5. Discover L3 v1 and L4 v1 are non-ranking diagnostics. Distance not production-promoted. L4 Class B / `phase_alignment` remain research shadow. L5 v1 is validated research shadow (mixed-state QI only; not ranking). Discover ranking is structural L2 (not CompatibilityScoring).  
