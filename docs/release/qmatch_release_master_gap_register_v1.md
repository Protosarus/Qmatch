...
| G-012 | banks | Dynamic IQ 25-session composer absent | **P2C-2A-2/3:** composer + **durable local resume** (`IqSessionManager` / SharedPreferences) **IMPLEMENTED_OFFLINE**. Live path still fixed 10-set | IMPLEMENTED_OFFLINE (runtime NOT_STARTED) | blocker | G-010 offline bank | `domain/iq_session/*`, prefs adapter | P2C-2A-4+ | Canonical scoring + production-called wiring | yes |
