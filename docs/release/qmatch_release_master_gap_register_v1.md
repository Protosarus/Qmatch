...
| G-008 | discover | Live Discover ranking is trusted structural L2 (`structural_l2_v1`); CompatibilityScoring remains **rollback only** (`legacy_v1`) | `discover_service.dart`, `discover_ranking_mode.dart`, `compareStageB2Structural` | **RESOLVED** (2026-08-16 cutover). Legacy path gated, not live | — | G-006 is separate (CM v2 still offline) | those files | P2C-3 ranking cutover done | Ranking uses trusted 20D L2; legacy path gated; no live %; Persona/archetype not rank keys | no |
| G-009 | discover | L3 v1: age/interests are production **diagnostics** (non-ranking); `looking_for` inactive; distance evaluated but **not production-promoted** (location privacy). Must not become L1 filters | L3 contract freeze 2026-08-16; attach after L2; order unchanged | **PARTIAL** — diagnostics frozen; ranking RFC + distance privacy + intent RFC open | high | L3 signal contract; location privacy | `l3_soft_preference_signal_*`, `discover_l3_soft_preference_shadow.dart` | later RFC | No L3 reorder; age mismatch ≠ unknown; distance not promoted; looking_for inactive | no |
...
| G-012 | banks | Dynamic IQ 25-session composer absent | **P2C-2A-5/6:** live IQ + IQ→20D partial profile adapter **IMPLEMENTED**. Calibration + EQ/Freq profile still open | IMPLEMENTED (EQ/Freq/calib open) | high | G-010 bank | `iq_canonical_runtime_service.dart`, `iq_to_20d_runtime_adapter.dart` | P2C-2A-7+ | EQ/Freq canonical profile groups | yes |
...
| G-014 | scoring | TraitScoring→Canonical adapter planned_not_wired | **P2C-2A-8R2:** Frequency live + FrequencyTo20d **IMPLEMENTED** → **20/20** `canonical_profile_ready=true` (uncalibrated). Persona/Matching/quantum still open. | IMPLEMENTED | blocker | Persona/Matching later | frequency runtime + FrequencyTo20d | P2C-3 | Persona/Matching as separate phases | yes (Persona/Matching) |
...
| G-030 | persona | PersonaScoring not runtime; **not** a Discover ranking key | **P2C-3A-3:** offline large-scale shadow stress **COMPLETE** (seed 20260809, n=100000, aggregate `docs/persona/reports/persona_shadow_stress_v1_aggregate.json`); self_center_failures=0, overall unreachable=0, H_norm≈0.92. Distance-only reveal candidate noted (`TEMPERATURE_NOT_REQUIRED_FOR_DISTANCE_ONLY_REVEAL_V1`, `CONFIDENCE_NOT_REQUIRED_FOR_DISTANCE_ONLY_REVEAL_V1`) but `DISTANCE_ONLY_REVEAL_READY_FOR_PRODUCT_REVIEW=false`; explainability **BLOCKED_PERSONA_REASON_CODE_POLICY**; production reveal + Firestore persona write still **NOT_STARTED**. Live Discover ranking is structural L2; archetype affinity is `legacy_v1` rollback only | SHADOW_STRESS_VALIDATED / REVEAL_BLOCKED | high | product + explainability for reveal; Top-2 bands if UI-gated | persona shadow scorer; stress reports; `docs/persona/*` | P2C-3A+ | Persona explanation-only after production policies; not ranking input; no auto-reveal | yes |
...
| G-041 | discover_ui | Live L2 card must not present CompatibilityScoring % / archetype chips as product signals | **2026-08-16:** `structural_l2_v1` does not attach a % and hides category/archetype chips + hint. Rollback (`legacy_v1`) still shows legacy UI. Goldens may fixture rollback fields | **PARTIAL** — live L2 presentation isolated; rollback UI retained | medium | G-008 | `qmatch_candidate_card.dart`, `discover_service.dart` | P2C-3 | Live L2 must not invent a % or treat Persona/archetype as matching keys | no |
...
Note: Some blockers are conditional on product claims (e.g. G-006 if CM v2 is required for v1 marketing). Live Discover ranking is already trusted structural L2 (`structural_l2_v1`); do not treat CompatibilityScoring as the live ranker.
...
- **G-041** records that live L2 must not show a CompatibilityScoring % or treat archetype as a matching key. Goldens may still fixture rollback (`legacy_v1`) presentation.
