...
| G-012 | banks | Dynamic IQ 25-session composer absent | **P2C-2A-2/3/4:** composer + local resume + **canonical 4D uncalibrated scorer** **IMPLEMENTED_OFFLINE**. Live path still fixed 10-set / correct-count | IMPLEMENTED_OFFLINE (runtime NOT_STARTED) | blocker | G-010 offline bank | `domain/iq_session/*`, `domain/iq_scoring/*` | P2C-2A-5+ | Production-called wiring + calibration | yes |
