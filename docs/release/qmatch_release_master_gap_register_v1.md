...
| G-012 | banks | Dynamic IQ 25-session composer absent | **P2C-2A-5:** composer + resume + 4D scorer **live-wired** in `IQTestScreen`. Legacy 10-set retired from active new sessions | IMPLEMENTED (calibration/20D NOT_STARTED) | high | G-010 bank | `iq_canonical_runtime_service.dart`, `iq_test_screen.dart` | P2C-2A-6+ | 20D adapter + calibration | yes |
