...
| G-012 | banks | Dynamic IQ 25-session composer absent | **P2C-2A-2:** `IqSessionComposer` **IMPLEMENTED_OFFLINE** (7/6/6/6 + family uniqueness + seeded options). Live path still fixed 10-set | IMPLEMENTED_OFFLINE (runtime NOT_STARTED) | blocker | G-010 offline bank | `domain/iq_session/*`, exposure report | P2C-2A-3+ | Persistence + production-called wiring | yes |
