...
| G-030 | persona | PersonaScoring not runtime; archetype still matching | **P2C-3A-1:** 18 IDs + v2 20D prototypes structurally present but provisional; live reveal blocked on reliability/E_j/T/Top-2 policies (`docs/persona/qmatch_persona_canonical_audit_v1.md`). Frequency path still skips Persona; Discover still uses archetype | DUPLICATED / LEGACY_ACTIVE / POLICY_BLOCKED | high | reliability + E_j + T + Top-2 | persona domain; compatibility_scoring; `docs/persona/*` | P2C-3A+ | Persona explanation-only after policies resolve; not ranking input unless decided | yes |
