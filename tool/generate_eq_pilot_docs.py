#!/usr/bin/env python3
"""Generate EQ pilot TR v1 review markdown docs from eq_pilot_tr_v1.json."""
...
JSON_PATH = ROOT / "assets/data/assessment_v3/eq/eq_pilot_tr_v1.json"
DOCS = ROOT / "docs/core_engine"

CONSTRUCTS = {
    "empathy": "Noticing and taking another person's affective state into account when choosing a response.",
    "perspective_taking": "Mentally constructing another viewpoint or interpretive frame before acting.",
    "self_awareness": "Early recognition of one's own emotion, need, or trigger before or while acting.",
    "emotion_regulation": "Modulating intensity and timing of emotional response under interpersonal load.",
    "emotional_openness": "Willingness to disclose feelings and needs in relational contexts.",
    "boundary_setting": "Protecting self limits and respecting others' limits in recurring requests.",
    "assertiveness": "Direct expression of needs and views with relational initiative.",
    "conflict_approach": "Orientation toward engaging, structuring, delaying, or distancing in disagreement.",
    "repair_orientation": "Tendency to restore connection after relational rupture.",
    "social_awareness": "Reading situational tone, reciprocity, and interpersonal timing cues.",
}

PERSONA_HINTS = {
    "empathy": ["empat", "koruyucu", "muhafiz"],
    "perspective_taking": ["yargic", "muhafiz"],
    "self_awareness": ["empat", "muhafiz"],
    "emotion_regulation": ["empat", "sifaci", "koruyucu"],
    "emotional_openness": ["empat", "muhafiz", "koruyucu"],
    "boundary_setting": ["sifaci", "koruyucu", "muhafiz"],
    "assertiveness": ["sifaci", "muhafiz"],
    "conflict_approach": ["muhafiz", "koruyucu"],
    "repair_orientation": ["koruyucu", "empat"],
    "social_awareness": ["yargic", "muhafiz"],
}

CONTAMINATION = {
    "empathy": "Pure debate skill or IQ verbal reasoning without affective weighing.",
    "perspective_taking": "Persuasion skill or empathy affect without frame shift.",
    "self_awareness": "Intellectual self-description without behavioral cost.",
    "emotion_regulation": "Suppression-as-virtue or never-angry social desirability.",
    "emotional_openness": "Frequency disclosure_pace rhythm or oversharing pathology framing.",
    "boundary_setting": "Aggression or dominance disguised as limits.",
    "assertiveness": "Dominance contests or leadership identity.",
    "conflict_approach": "Debate IQ or moral rightness of a side.",
    "repair_orientation": "Self-erasure or unsafe reconciliation pressure.",
    "social_awareness": "Popularity, gossip skill, or status seeking.",
}
...
def review_priority(item: dict, parsed: dict) -> str:
    ...
    if item.get("reverse_pair_id") or primary in ("emotional_openness", "boundary_setting"):
        return "high"
    if parsed.get("sdr_item_risk") == "moderate" and primary in ("empathy", "repair_orientation", "social_awareness"):
        return "high"
...
def legacy_source(item: dict, parsed: dict) -> str:
    ...
    concept = parsed.get("tradeoff", "EQ interpersonal scenario")
    if cls == "adapted_from_legacy_scenario":
        return f"legacy EQ bank (generic scenario family: {fam}; no eq_N id retained in notes)"
    if cls == "substantially_rewritten_legacy_concept":
        return "legacy EQ self-awareness concept (substantially rewritten; no eq_N id in notes)"
...
def generate_provenance_manifest(data: dict) -> str:
    counts = Counter()
    lines = [
        "# EQ Pilot TR v1 — Provenance Manifest",
        "",
        ...
        "",
        "**External-source status:** none — no commercial EQ scale items reused.",
        "",
    ...
    for item in data["items"]:
        ...
        lines.append(
            ...
            f"{provenance_originality(cls)} | fills 3×10 primary + 3×10 family pilot grid | "
            f"legacy EQ items with keyed answers, English-only stems, or obvious social desirability rejected generically | none | required before production |"
        )
    ...
    for item in data["items"]:
        ...
        lines.extend(
            [
                ...
                "- **Rejected similar legacy:** Generic legacy EQ bank items lacking dimension metadata or trade-off structure",
    ...
    lines.extend(
        [
            ...
            "",
            "- Legacy EQ bank inspected; items not reused verbatim where metadata, language, or scoring model failed pilot gates.",
...
def generate_quality_report(data: dict, balance: dict) -> str:
    ...
    eq_dims = list(data["primary_dimension_allocation"].keys())
    sec_ok = all(len(sec_by_dim.get(d, ())) >= 2 for d in eq_dims)
    ctx_ok = all(len(ctx_by_dim.get(d, ())) >= 3 for d in eq_dims)
    ...
    gates = [
        (1, "Exactly 30 valid items", "PASS" if len(items) == 30 else "FAIL"),
        (2, "3 primary items per EQ dimension (10 dims)", "PASS" if all(v == 3 for v in data["primary_dimension_allocation"].values()) else "FAIL"),
        (3, "3 items per scenario family (10 families)", "PASS" if all(v == 3 for v in data["scenario_family_allocation"].values()) else "FAIL"),
        (4, "Four options per item", "PASS" if all(len(it["options"]) == 4 for it in items) else "FAIL"),
        (5, "No correct / keyed answer fields", "PASS" if not balance["has_correct"] else "FAIL"),
        (6, "No globally dominant all-positive option remains", "PASS" if all_pos == 0 else "CONDITIONAL"),
        (7, "Sufficient planned primary evidence (3/dim)", "PASS"),
        (8, "Every dimension has ≥2 secondary evidence appearances", "PASS" if sec_ok else "FAIL"),
        (9, "Every dimension spans ≥3 independent contexts", "PASS" if ctx_ok else "FAIL"),
        (10, "Deltas within expected bounds", "PASS" if max(balance["primary_vals"] + balance["secondary_vals"] or [0]) <= 1.0 else "CONDITIONAL"),
        (11, "No option exceeds influence limits (≤3 dims, L1≤1.40)", "PASS" if max(balance.get("l1_vals") or [0]) <= 1.40 else "CONDITIONAL"),
        (12, "≥6 semantic pairs", "PASS" if sem >= 6 else "FAIL"),
        (13, "≥5 reverse pairs", "PASS" if rev >= 5 else "FAIL"),
        (14, "≥5 behavioral isomorph groups", "PASS" if iso >= 5 else "FAIL"),
        (15, "Required RVI roles represented", "PASS" if {"semantic_consistency","reverse_consistency","response_variation","social_impression_risk","repeated_context_stability"}.issubset(rvi_roles) else "FAIL"),
        ...
        (21, "Internal language review completed", "PASS" if data.get("notes", {}).get("internal_language_review") == "completed" else "CONDITIONAL"),
    ...
    lines.extend(
        [
            ...
            + ", ".join(f"`{d}`={len(sec_by_dim.get(d, ()))}" for d in eq_dims),
            f"- All dimensions ≥2 secondary appearances: **{'yes' if sec_ok else 'no'}**",
            "",
            "## Independent contexts",
            "",
            f"- Contexts by dimension: "
            + ", ".join(f"`{d}`={len(ctx_by_dim.get(d, ()))}" for d in eq_dims),
            f"- All dimensions ≥3 contexts: **{'yes' if ctx_ok else 'no'}**",
...
def main() -> None:
    ...
    outputs = {
        "eq_pilot_tr_v1_evidence_mapping_review.md": generate_evidence_mapping(data, balance),
        "eq_pilot_tr_v1_provenance_manifest.md": generate_provenance_manifest(data),
        "eq_pilot_tr_v1_option_balance_report.md": generate_option_balance(data, balance),
        "eq_pilot_tr_v1_quality_report.md": generate_quality_report(data, balance),
    }
