...
/// Phase 3C soft-diversity audit: 10k-seed simulation of the dormant V2 selector.
///
...
void main(List<String> args) {
  var sessions = 10000;
  var outPath = FrequencyBehaviorV2Contract.phase3cSimulationReportRelativePath;
  ...
  final clusterOf = <String, String>{
    for (final id in selectableIds) id: pool.itemsById[id]!.semanticCluster,
  };
  final poolByDim = <String, int>{
    for (final d in FrequencyBehaviorV2Contract.canonicalDimensions) d: 0,
  };
  final clusterPool = <String, Map<String, int>>{
    for (final d in FrequencyBehaviorV2Contract.canonicalDimensions) d: {},
  };
  for (final id in selectableIds) {
    final dim = dimOf[id]!;
    poolByDim[dim] = (poolByDim[dim] ?? 0) + 1;
    final cluster = clusterOf[id]!;
    clusterPool[dim]![cluster] = (clusterPool[dim]![cluster] ?? 0) + 1;
  }
  ...
  final occupancyGroups = <int, int>{1: 0, 2: 0, 3: 0, 4: 0};
  final sessionMaxOccupancy = <int, int>{1: 0, 2: 0, 3: 0, 4: 0};
  final clusterPicks = <String, Map<String, int>>{
    for (final d in FrequencyBehaviorV2Contract.canonicalDimensions) d: {},
  };
  final clusterSessions = <String, Map<String, int>>{
    for (final d in FrequencyBehaviorV2Contract.canonicalDimensions) d: {},
  };
  var adjacentSame = 0;
  var adjacentPairs = 0;
  ...
  for (var n = 0; n < sessions; n++) {
    ...
    final selected = m.questionIds.toSet();
    for (final group in clusters) {
      var hit = 0;
      for (final id in group) {
        if (selected.contains(id)) hit++;
      }
      if (hit >= 2) nearDupViolations++;
    }
    final counts = <String, int>{
      for (final d in FrequencyBehaviorV2Contract.canonicalDimensions) d: 0,
    };
    final dimCluster = <String, Map<String, int>>{};
    var run = 1;
    var hadTwo = false;
    for (var i = 0; i < m.questions.length; i++) {
      ...
      final cluster = clusterOf[q.questionId] ?? '';
      dimCluster.putIfAbsent(q.primaryDimension, () => {});
      dimCluster[q.primaryDimension]![cluster] =
          (dimCluster[q.primaryDimension]![cluster] ?? 0) + 1;
    ...
    var maxOcc = 1;
    for (final dimEntry in dimCluster.entries) {
      for (final e in dimEntry.value.entries) {
        final nPick = e.value;
        occupancyGroups[nPick >= 4 ? 4 : nPick] =
            (occupancyGroups[nPick >= 4 ? 4 : nPick] ?? 0) + 1;
        if (nPick > maxOcc) maxOcc = nPick;
        clusterPicks[dimEntry.key]![e.key] =
            (clusterPicks[dimEntry.key]![e.key] ?? 0) + nPick;
        clusterSessions[dimEntry.key]![e.key] =
            (clusterSessions[dimEntry.key]![e.key] ?? 0) + 1;
      }
    }
    sessionMaxOccupancy[maxOcc >= 4 ? 4 : maxOcc] =
        (sessionMaxOccupancy[maxOcc >= 4 ? 4 : maxOcc] ?? 0) + 1;
    final fives = [
  ...
  String formerRows() {
    ...
    for (final id in formerAlways) {
      ...
      final dim = dimOf[id]!;
      final expectedPct = 100.0 * expectedAlloc / poolByDim[dim]!;
      final ratio = (100.0 * n / sessions) / expectedPct;
      buf.writeln(
        '- `$id` ($dim, cluster=`${clusterOf[id]}`): $n (${pct(n)}%); '
        'dim expected ${expectedPct.toStringAsFixed(2)}%; '
        'ratio ${ratio.toStringAsFixed(2)}',
      );
    }
  ...
  String occupancyRows() {
    final buf = StringBuffer();
    buf.writeln(
      'Occupancy groups (session × dimension × cluster with at least one pick):',
    );
    buf.writeln('- 1: ${occupancyGroups[1]}');
    buf.writeln('- 2: ${occupancyGroups[2]}');
    buf.writeln('- 3: ${occupancyGroups[3]}');
    buf.writeln('- 4+: ${occupancyGroups[4]}');
    buf.writeln('');
    buf.writeln(
        'Sessions whose max same-cluster occupancy (within a dimension) is:');
    buf.writeln('- 1: ${sessionMaxOccupancy[1]}');
    buf.writeln('- 2: ${sessionMaxOccupancy[2]}');
    buf.writeln('- 3: ${sessionMaxOccupancy[3]}');
    buf.writeln('- 4+: ${sessionMaxOccupancy[4]}');
    return buf.toString().trimRight();
  }

  String clusterRepRows() {
    final buf = StringBuffer();
    for (final d in FrequencyBehaviorV2Contract.canonicalDimensions) {
      buf.writeln('### `$d` (pool ${poolByDim[d]})');
      buf.writeln('');
      buf.writeln(
        '| cluster | pool | sessions_with_≥1 | pct | total_picks |',
      );
      buf.writeln('|---|---:|---:|---:|---:|');
      final names = clusterPool[d]!.keys.toList()
        ..sort((a, b) {
          final c =
              (clusterPool[d]![b] ?? 0).compareTo(clusterPool[d]![a] ?? 0);
          if (c != 0) return c;
          return a.compareTo(b);
        });
      for (final name in names) {
        final nPool = clusterPool[d]![name] ?? 0;
        final nSess = clusterSessions[d]![name] ?? 0;
        final nPick = clusterPicks[d]![name] ?? 0;
        final label = name.isEmpty ? '(empty)' : name;
        buf.writeln(
          '| `$label` | $nPool | $nSess | ${pct(nSess)} | $nPick |',
        );
      }
      buf.writeln('');
    }
...
Seeds: `phase3c-sim-0` … `phase3c-sim-${sessions - 1}`
Sessions: **$sessions**
Soft cluster lookahead: **${FrequencyBehaviorV2Contract.softClusterLookahead}**
...
- Near-duplicate co-selection violations: **$nearDupViolations**
