// Offline deterministic validator for Frequency pilot form (P2A-2D-1).
// Usage: dart run tool/validate_frequency_pilot_v1.dart
...
const freqDims = PersonaDimensionIds.frequency;
...
void main(List<String> args) {
  ...
  if (form['form_id'] != 'frequency_tr_pilot_v1') {
    err('form_id', 'expected frequency_tr_pilot_v1');
  }
  if (form['module'] != 'frequency') {
    err('module', 'module must be frequency');
  }
  ...
  if (form['content_version'] != 'frequency-tr-pilot-v1') {
    err('content_version', 'expected frequency-tr-pilot-v1');
  }
  ...
  if (form['set_id'] != 'frequency_tr_pilot_v1_set_001') {
    err('set_id', 'expected frequency_tr_pilot_v1_set_001');
  }
  if (form['question_count'] != 50) {
    err('question_count', 'question_count must be 50');
  }

  final items = (form['items'] as List?) ?? const [];
  if (items.length != 50) {
    err('item_count', 'expected 50 items, got ${items.length}');
  }
  ...
  final primaryAllocation = Map<String, dynamic>.from(
    form['primary_dimension_allocation'] as Map? ?? {},
  );
  ...
  final contextsByDim = {for (final d in freqDims) d: <String>{}};
  final evidenceStrengthValues = <double>{};
  ...
  for (final raw in items) {
    ...
    if (!idPattern.hasMatch(qid)) {
      err('question_id_pattern', 'expected frequency_tr_v1_<dim>_00N',
          qid: qid);
    }
    if (j['module'] != 'frequency') {
      err('module', 'item module must be frequency', qid: qid);
    }
    ...
    for (final oRaw in options) {
      ...
      final est = (o['evidence_strength'] as num?)?.toDouble() ?? 0.0;
      if (est < 0.40 || est > 0.85) {
        err('evidence_strength_range',
            'evidence_strength=$est out of [0.40,0.85]',
            qid: qid);
      }
      if ((est - 0.72).abs() < 1e-9) {
        warn(
            'evidence_strength_072', 'evidence_strength uses flat 0.72 default',
            qid: qid);
      }
      evidenceStrengthValues.add(est);
      ...
      final primaryDelta = (deltas[domain] as num?)?.toDouble() ?? 0.0;
      if ((est - primaryDelta.abs()).abs() < 1e-9) {
        err('evidence_strength_equals_primary',
            'evidence_strength must not equal |primary delta|',
            qid: qid);
      }
      var l1 = 0.0;
      var nonzero = 0;
      for (final de in deltas.entries) {
        ...
        if (!freqDims.contains(de.key) && de.key.isNotEmpty) {
          err('delta_non_frequency', de.key, qid: qid);
        }
        if (eqDims.contains(de.key)) {
          err('delta_eq_leakage', 'EQ dimension ${de.key} in frequency deltas',
              qid: qid);
        }
        if (de.key == 'emotional_openness') {
          err('delta_emotional_openness',
              'emotional_openness forbidden in frequency bank',
              qid: qid);
        }
  ...
  for (final d in freqDims) {
    final expected = (primaryAllocation[d] as num?)?.toInt();
    if (expected == null) {
      err('primary_allocation_missing', 'missing allocation for $d');
    } else if (domainCounts[d] != expected) {
      err('primary_allocation',
          'expected $expected primary for $d got ${domainCounts[d]}');
    }
    if ((secondaryAppearances[d] ?? 0) < 5) {
      err('secondary_evidence', '$d secondary appearances < 5');
    }
    if ((contextsByDim[d]?.length ?? 0) < 5) {
      err('independent_contexts', '$d has < 5 scenario families');
    }
  }
  ...
  if (evidenceStrengthValues.length < 2) {
    err('evidence_strength_flat', 'evidence_strength not varied across bank');
  }
  if (evidenceStrengthValues.length == 1 &&
      evidenceStrengthValues.first == 0.72) {
    err('evidence_strength_072_flat', 'all options use flat 0.72');
  }
  ...
  if (semRegistry.length < 8) {
    err('semantic_pairs', 'expected >=8 semantic pairs');
  }
  if (revRegistry.length < 6) {
    err('reverse_pairs', 'expected >=6 reverse pairs');
  }
  if (isoRegistry.length < 6) {
    err('isomorph_groups', 'expected >=6 isomorph groups');
  }
  ...
  try {
    ...
    const testDim = 'disclosure_pace';
    final highDisclosure = svc.scoreModule(
      sess(keyed(
        (q) => q.primaryDimension == testDim
            ? pickMax(q, testDim)
            : pickModerate(q),
      )),
    );
    final lowDisclosure = svc.scoreModule(
      sess(keyed(
        (q) => q.primaryDimension == testDim
            ? pickMin(q, testDim)
            : pickModerate(q),
    ...
    if (!highDisclosure.module.dimensionScores.containsKey(testDim) ||
        !lowDisclosure.module.dimensionScores.containsKey(testDim)) {
      err('disclosure_pace_scores', 'disclosure_pace not scored');
    } else if (highDisclosure.module.dimensionScores[testDim]! <=
        lowDisclosure.module.dimensionScores[testDim]!) {
      err('negative_evidence',
          'high disclosure_pace should exceed low disclosure_pace');
    }

    final omitDisclosure = [
      for (final q in parsed)
        if (q.primaryDimension != testDim)
          AssessmentResponse(
    ...
    final miss = svc.scoreModule(sess(omitDisclosure));
    if (miss.module.dimensionScores.containsKey(testDim)) {
      err('missing_published',
          'disclosure_pace should be missing when primaries omitted');
    }
    if (!miss.module.missingDimensions.contains(testDim) &&
        !miss.module.insufficientDimensions.contains(testDim)) {
      err('missing_flag', 'disclosure_pace not flagged missing/insufficient');
    }

    final incomplete = svc.scoreModule(
      sess(keyed(pickModerate).take(10).toList(), status: 'incomplete'),
    );
    ...
    info('trait_scoring',
        'Frequency pilot scoring smoke passed (${parsed.length} items)');
  } catch (e) {
  ...
  report['evidence_strength_values'] = evidenceStrengthValues.toList()..sort();
  ...
  report['notes'] = {
    ...
    'eq_construct_separation':
        'no EQ deltas; disclosure_pace not emotional_openness',
  };

  final outDir = Directory('$root/tool/frequency_pilot_out');
  if (!outDir.existsSync()) outDir.createSync(recursive: true);
  final outFile =
      File('${outDir.path}/validate_frequency_pilot_v1_report.json');
  final encoded = const JsonEncoder.withIndent('  ').convert(_sortJson(report));
  outFile.writeAsStringSync('$encoded\n');

  stdout.writeln('=== Frequency Pilot Validator (P2A-2D-1) ===');
