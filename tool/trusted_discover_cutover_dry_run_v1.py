#!/usr/bin/env python3
"""Trusted Discover cutover — DRY-RUN only.

Compares stored discover_eligible against the NEW trusted Discover formula.
...
from trusted_discover_cutover_policy_v1 import (  # noqa: E402
    classify_cutover,
    empty_counts,
    increment_cutover_counts,
    public_counts,
)
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Dry-run trusted Discover cutover (read-only counts)."
    )
    p.add_argument(
        "--execute-dry-run",
        action="store_true",
...
def print_usage() -> None:
    print(
        ...
        "  python3 tool/trusted_discover_cutover_dry_run_v1.py --execute-dry-run\n"
        "\n"
        "Never writes users documents. Never triggers Cloud Functions.\n"
        "Do NOT deploy Discover or rules in this phase.\n"
    )
...
def run_dry_run(*, page_size: int, write_local_report: bool) -> int:
    ...
    for doc in db.collection(COLLECTION).stream():
        ...
        classified = classify_cutover(data)
        increment_cutover_counts(counts, classified)
    ...
    if write_local_report:
        ...
        out = BUILD_DIR / "trusted_discover_cutover_dry_run_v1.json"
