...
/// Offline helpers for EQ review candidate 1 (not production).
class EqPilotReviewCandidate1Loader {
  EqPilotReviewCandidate1Loader._();

  static const candidatePath =
      'assets/data/assessment_v3/eq/eq_pilot_tr_v1_review_candidate_1.json';
  static const parentPath = 'assets/data/assessment_v3/eq/eq_pilot_tr_v1.json';
  ...
  static List<AssessmentItemDefinition> loadItems(TraitScoringConfig config) {
    ...
    return TraitScoringParser.parseItemBank(
      ...
      expectedModule: 'eq',
      source: 'eq_pilot_tr_v1_review_candidate_1.json',
      config: config,
    );
  }
    ...
    List<AssessmentItemDefinition> items,
      ...
      fromPicker(items, (q) {
        if (q.primaryDimension == 'boundary_setting') {
          return pickMax(q, 'boundary_setting');
        }
        if (q.primaryDimension == 'emotional_openness') {
          return pickMin(q, 'emotional_openness');
        }
  ...
  static List<AssessmentResponse> highOpennessLowBoundary(
    List<AssessmentItemDefinition> items,
      ...
      fromPicker(items, (q) {
        if (q.primaryDimension == 'emotional_openness') {
          return pickMax(q, 'emotional_openness');
        }
        if (q.primaryDimension == 'boundary_setting') {
          return pickMin(q, 'boundary_setting');
        }
        return pickModerate(q);
      });

  static List<AssessmentResponse> highAssertivenessLowRepair(
    List<AssessmentItemDefinition> items,
  ) =>
      fromPicker(items, (q) {
        if (q.primaryDimension == 'assertiveness') {
          return pickMax(q, 'assertiveness');
        }
        if (q.primaryDimension == 'repair_orientation') {
          return pickMin(q, 'repair_orientation');
        }
        return pickModerate(q);
      });

  static List<AssessmentResponse> highRepairLowAssertiveness(
    List<AssessmentItemDefinition> items,
  ) =>
      fromPicker(items, (q) {
        if (q.primaryDimension == 'repair_orientation') {
          return pickMax(q, 'repair_orientation');
        }
        if (q.primaryDimension == 'assertiveness') {
          return pickMin(q, 'assertiveness');
        }
        return pickModerate(q);
      });

  static List<AssessmentResponse> privacyOriented(
    List<AssessmentItemDefinition> items,
  ) =>
      fromPicker(items, (q) {
        if (q.primaryDimension == 'emotional_openness' ||
            q.primaryDimension == 'empathy') {
          return pickMin(q, q.primaryDimension);
        }
        if (q.primaryDimension == 'boundary_setting') {
          return pickMax(q, 'boundary_setting');
        }
  ...
  static List<AssessmentResponse> directConflict(
    List<AssessmentItemDefinition> items,
  ) =>
      fromPicker(items, (q) {
        if (q.primaryDimension == 'conflict_approach' ||
            q.primaryDimension == 'assertiveness') {
          return pickMax(q, q.primaryDimension);
        }
        return pickModerate(q);
      });

  static List<AssessmentResponse> coolingOff(
    List<AssessmentItemDefinition> items,
  ) =>
      fromPicker(items, (q) {
        if (q.primaryDimension == 'emotion_regulation') {
          return pickMax(q, 'emotion_regulation');
        }
        if (q.primaryDimension == 'conflict_approach') {
          return pickMin(q, 'conflict_approach');
        }
        return pickModerate(q);
      });
  ...
  /// Trait-consistent reverse pattern after behavioral keying:
  /// same primary sign on both reverse-pair members.
  static List<AssessmentResponse> reverseTraitConsistent(
    List<AssessmentItemDefinition> items,
  ) =>
      fromPicker(items, (q) {
  ...
  static List<AssessmentResponse> reverseTraitInconsistent(
    List<AssessmentItemDefinition> items,
  ) {
    final seen = <String>{};
    return fromPicker(items, (q) {
  ...
  static List<AssessmentResponse> idealizedHighSdr(
    List<AssessmentItemDefinition> items,
  ) =>
      fromPicker(items, (q) {
        var best = q.options.first;
        var bestScore = double.negativeInfinity;
        for (final o in q.options) {
          final primary = o.dimensionDeltas[q.primaryDimension] ?? 0.0;
          final sdrBoost = o.socialDesirabilityRisk == 'moderate' ? 1.0 : 0.0;
          final score = primary + sdrBoost;
          if (score > bestScore) {
            bestScore = score;
            best = o;
          }
