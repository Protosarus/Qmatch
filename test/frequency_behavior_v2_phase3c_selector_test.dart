...
void main() {
  ...
  test('same selector_version + bank_version + session_seed is bit-stable', () {
    final a = compose('phase3c-det-1', createdAt: '2026-01-01T00:00:00Z');
    final b = compose('phase3c-det-1', createdAt: '2026-12-31T23:59:59Z');
  ...
  test('DROP questions are never selected', () {
    ...
    for (var i = 0; i < 40; i++) {
      final m = compose('phase3c-drop-$i');
      ...
      for (var j = 1; j < m.questions.length; j++) {
        if (m.questions[j].primaryDimension ==
          ...
          expect(
            ...
            reason: 'seed=phase3c-run-$i at $j',
          );
        } else {
          run = 1;
        }
  ...
  test(
    ...
    final again = compose('phase3c-opts');
    expect(
  ...
  test('stored answers reference option_id; display index is not an identity',
      () {
    final m = compose('phase3c-score-id');
  ...
  test(
      '200-seed sample: no 100% item, no near-dup pair, large clusters may exceed 2',
      () {
    final extra = <String, int>{
      for (final d in FrequencyBehaviorV2Contract.canonicalDimensions) d: 0,
    };
    final freq = <String, int>{};
    var sawThreeFromSameCluster = false;
    const sessions = 200;
    for (var i = 0; i < sessions; i++) {
      final m = compose('phase3c-sample-$i');
      _assertCoverage(m);
      ...
      for (final group in clusters) {
        ...
        expect(hit, lessThan(2),
            reason: 'near-dup $group in phase3c-sample-$i');
      }
      final counts = <String, int>{};
      ...
      for (final q in m.questions) {
        ...
        final cluster = pool.itemsById[q.questionId]!.semanticCluster;
      ...
      for (final clustersForDim in dimCluster.values) {
        if (clustersForDim.values.any((n) => n >= 3)) {
          sawThreeFromSameCluster = true;
        }
      }
    }
    expect(extra.values.where((n) => n > 0).length, 12);
    expect(freq.values.reduce((a, b) => a > b ? a : b), lessThan(sessions));
    expect(sawThreeFromSameCluster, isTrue,
        reason: 'hard max-2 cluster cap must be gone');
    for (final id in _formerHardCapInflated) {
      final n = freq[id] ?? 0;
  ...
  test('10k soft-diversity report exists with coverage and no 100% items', () {
    final report = File(
      '${Directory.current.path}/${FrequencyBehaviorV2Contract.phase3cSimulationReportRelativePath}',
    ).readAsStringSync();
    ...
    expect(
      report.contains('Near-duplicate co-selection violations: **0**'),
      isTrue,
    );
