#!/usr/bin/env python3
"""Apply Phase 3L-A3 EQ content rewrite to eq_set_021..030.
...
SCENARIOS_PATH = ROOT / "scripts" / "eq_batch3_scenarios.json"

SECURE = [
    ("Stay calm, thank them for saying it, and ask what's underneath", "Sakin kalır, söyledikleri için teşekkür eder ve altında ne olduğunu sorarsın"),
    ("Hear the worry and ask what would actually help right now", "Endişeyi dinler ve şu anda gerçekten neyin yardımcı olacağını sorarsın"),
    ("Take it private, name how it landed, and reset the moment kindly", "Konuyu özelde açar, nasıl geldiğini söyler ve anı nazikçe toparlarsın"),
    ("Bring it up one-to-one without turning it into a public trial", "Herkese açık bir yargıya çevirmeden bire bir konuşursun"),
    ...
    ("Thank them for openness and ask how to repair this well", "Açıklıkları için teşekkür edip bunu nasıl onarabileceğinizi sorarsın"),
...
def classify(opt: str) -> str:
    ...
    if any(
        k in t
        for k in [
            "stay steady, thank them",
            "validate the worry",
            "privately tell them it landed",
            "address it directly in private",
        ]
    ):
        return "secure"
    if "dismiss it as irrational" in t:
        return "defensive"
    if any(
        k in t
        for k in [
...
def main() -> None:
    ...
    for s in data["sets"]:
        if not re.match(r"^eq_set_02[1-9]$|^eq_set_030$", s["id"]):
            continue
    ...
    print(f"Rewrote batch 3: {q_counter} questions in eq_sets.json")
