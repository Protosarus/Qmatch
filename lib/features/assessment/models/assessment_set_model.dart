...
class AssessmentAssignmentModel {
  ...
  /// `en` | `tr` — language used when assignment was created/opened.
  final String? languageUsed;
  /// e.g. `tr_TR` / `en_US`
  final String? localeUsed;
  /// When runtime recovered from an obsolete English-only legacy assignment.
  final String? recoveredFromLegacySetId;
  final Timestamp? recoveredAt;
  ...
  factory AssessmentAssignmentModel.fromFirestore(Map<String, dynamic> data) {
    return AssessmentAssignmentModel(
      ...
      languageUsed: (data['language_used'] as String?)?.trim(),
      localeUsed: (data['locale_used'] as String?)?.trim(),
      recoveredFromLegacySetId:
          (data['recovered_from_legacy_set_id'] as String?)?.trim(),
      recoveredAt: data['recovered_at'] is Timestamp
          ? data['recovered_at'] as Timestamp
          : null,
    );
  }
  ...
  Map<String, dynamic> toFirestoreNewAssignment() {
    return {
      ...
      if (languageUsed != null && languageUsed!.isNotEmpty)
        'language_used': languageUsed,
      if (localeUsed != null && localeUsed!.isNotEmpty)
